...
export default async function Page() {
  ...
  const findChampionshipIdForDateKey = (key: string) => {
    if (!key || key === 'Sin-fecha') return 'none'
    const champ = championships.find((c) => {
      if (!c.startDate) return false
      if (key < c.startDate) return false
      if (c.endDate && key > c.endDate) return false
      return true
    })
    return champ?.id ?? 'none'
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        const byChamp = new Map<string, typeof sessions>()
        for (const s of sessionsForViewer) {
          const key = sessionDateKey(s)
          const champId = findChampionshipIdForDateKey(key)
          const arr = byChamp.get(champId) ?? []
          arr.push(s)
          byChamp.set(champId, arr)
        }

        const champOrder: string[] = []
        for (const c of championships) champOrder.push(c.id)
        if (byChamp.has('none')) champOrder.push('none')

        return champOrder
          .filter((id) => byChamp.has(id))
          .map((champId) => {
            const listAll = byChamp.get(champId) ?? []
            if (listAll.length === 0) return null
            const champ = championships.find((c) => c.id === champId)

            const grouped = new Map<string, typeof listAll>()
            ...
            return (
              <div key={champId} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-base md:text-lg font-bold">
                    {champ ? champ.title : 'Sesiones sin campeonato'}
                  </div>
                  {champ && (
                              ...
                              <div className="space-y-0.5">
                                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                                  ...
                                  {publishedSet.has(canonicalId(s.id)) ? (
                                    <Badge
                                      className="text-xs px-2 py-0.5 bg-[#2b855d] text-white"
                                      variant="default"
                                    >
                                      Resultado oficial
                                    </Badge>

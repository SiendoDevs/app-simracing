...
export const metadata = {
  title: "ZN Kart SimRacing | 2026 – Português",
  description: "Resultados oficiais, campeonatos e servidor público da liga ZN Kart SimRacing.",
};

export default async function HomePortuguese() {
  ...
  const labelType = (t: string) => {
    ...
    if (up === "RACE") return "Corrida";
    if (up === "QUALIFY") return "Classificação";
    if (up === "PRACTICE") return "Treino";
    return t;
  };
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 items-stretch">
        <div className="rounded-lg border p-3 md:p-4 h-full flex flex-col">
          <div className="flex-1 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <div className="text-sm font-semibold">Inscrição</div>
                <p className="text-sm text-muted-foreground">{currentChampionship.rules.inscription}</p>
                <div className="text-sm font-semibold">Vagas</div>
        ...
        <div className="rounded-lg border h-full flex flex-col">
          <div className="px-3 md:px-4 py-2 text-sm md:text-base font-semibold border-b">
            Datas do campeonato
          </div>
      ...
      <ul className="rounded-lg border divide-y">
        {sessionsRecent.length === 0 && (
          <li className="p-4 text-sm text-muted-foreground">Nenhum arquivo JSON de sessão encontrado.</li>
        )}
        {sessionsRecent
          .slice()
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    {s.type.toUpperCase() === "RACE"
                      ? `Corrida ${raceIndexMap.get(s.id) ?? 1}`
                      : labelType(s.type)}
                  </Badge>
      ...
      <section className="rounded-xl border bg-muted/40 p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
        <div className="space-y-3">
          <h2 className="text-2xl md:text-3xl font-bold inline-flex items-center gap-3">
            ...
            Entre no servidor público
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl">
            Treine, se divirta e corra com amigos 24 horas por dia. Nosso servidor está aberto para todos, sem senha ou
            inscrição prévia.
          </p>
          <p className="text-sm font-medium text-foreground/80">
            Procure por <span className="text-[#d8552b] font-bold">&quot;ZN Simracing&quot;</span> no Content Manager e
            comece a girar agora mesmo.
          </p>
        </div>
